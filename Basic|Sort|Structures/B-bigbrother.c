...
void tree_build(Node* tree,int* arr, int idx, int left, int right)
{
	if (left == right)
	{
		tree[idx].max_val = arr[left];
		tree[idx].xor_val = arr[left];
	}
	else
	{
		int middle = left + (right - left) / 2;
		tree_build(tree, arr, 2 * idx, left, middle);
		tree_build(tree, arr, 2 * idx + 1, middle + 1, right);
